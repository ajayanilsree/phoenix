...
@role_required("customer")
def address(request):
    saved_address = Address.objects.filter(user=request.user, is_default=True).order_by("-updated_at", "-id").first()
    is_editing = request.method == "POST" or request.GET.get("edit") == "1" or saved_address is None
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Address.objects.filter(user=request.user, is_default=True).exclude(pk=customer_address.pk).update(is_default=False)
